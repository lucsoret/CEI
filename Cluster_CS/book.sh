...
{
    if [ $mode == 0 ]; then
	ssh $login@term2.grid "oarsub -r \"$(date +'%F %T')\" -p \"network_address in ('sh01', 'sh02', 'sh03', 'sh04','sh05','sh06', 'sh07','sh08','sh09','sh10','sh11', 'sh12','sh13','sh14','sh15','sh16','cam00', 'cam01', 'cam02', 'cam03', 'cam04','cam05','cam06', 'cam07','cam08','cam09','cam10','cam11', 'cam12','cam13')\" -l nodes=1,walltime=24:00:00" > reservation$job_num.log
    else
	ssh "$ssh_options" $login@term2.grid "oarsub -r \"$(date +'%F %T')\" -p \"network_address in ('sh01', 'sh02', 'sh03', 'sh04','sh05','sh06', 'sh07','sh08','sh09','sh10','sh11', 'sh12','sh13','sh14','sh15','sh16','cam00', 'cam01', 'cam02', 'cam03', 'cam04','cam05','cam06', 'cam07','cam08','cam09','cam10','cam11', 'cam12','cam13')\" -l nodes=1,walltime=24:00:00" > reservation$job_num.log
    fi    
}
